...
const nativeSelectClass =
	"rounded-xl border border-separator bg-content1 px-4 py-2.5 text-sm text-foreground outline-none transition-all focus:border-primary appearance-none";
...
export default function DispatchSection({
			...
			await invoke("upsert_dispatch_job", {
				input: {
					...
					max_time_before_reset_minutes: Number(
						form.max_time_before_reset_minutes,
					),
				},
			});
			setForm(defaultForm);
	...
	async function handleToggle(job: DispatchJob) {
		...
		setError(null);
		try {
			const input: DispatchJobEnabledInput = {
				id: job.id,
				enabled: !job.enabled,
			};
			...
			setError(
				typeof toggleError === "string" ? toggleError : String(toggleError),
			);
		} finally {
			setBusyId(null);
		}
	...
	async function handleDelete(jobId: string) {
		...
		try {
			...
			setError(
				typeof deleteError === "string" ? deleteError : String(deleteError),
			);
		} finally {
			setBusyId(null);
		}
				...
				<CardContent className="p-3 flex flex-col gap-2">
					...
					{dispatchState.error && !dispatchState.data ? (
						...
						<>
							...
							{nextSummary && (
								<p className="text-xs text-foreground">{nextSummary}</p>
							)}
						</>
					)}
				</CardContent>
		...
		<div className="flex flex-col gap-6">
			...
			{showForm ? (
				<Card className="w-full border-primary/20 shadow-lg shadow-primary/5">
					<CardContent className="p-2 flex flex-col gap-6">
						<div className="flex items-center justify-between">
						...
						>
									...
									onChange={(event) =>
										setForm((current) => ({
											...
											schedule_kind: event.target
												.value as DispatchJob["schedule_kind"],
										}))
									}
								>
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
								<label className={fieldLabelClass}>
									...
									<Input
										...
										aria-label="Minimum budget percent"
									/>
								</label>
								<label className={fieldLabelClass}>
									Buffer (Mins)
							...
							<div className="flex flex-col gap-4 pt-2 md:col-span-2 sm:flex-row sm:items-center sm:justify-between">
								...
								>
									...
									<Switch.Content>Initially Enabled</Switch.Content>
								</Switch>
								...
								<div className="flex items-center justify-end gap-3">
									...
									<Button
										...
										variant="primary"
										size="md"
										className="font-semibold px-8 flex-1 sm:flex-none"
										isDisabled={saving}
									>
										{saving
											? "Saving..."
				...
				<Card className="w-full border-dashed border-2 border-separator bg-transparent">
					<CardContent className="flex flex-col items-center justify-center py-16 px-6 text-center gap-4">
						...
						<div className="flex flex-col gap-1 max-w-sm">
							...
							<p className="text-sm text-muted">
								Create a job to automatically run commands when you have
								remaining budget right before it resets.
							</p>
						</div>
						<Button
							variant="secondary"
				...
				<div className="flex flex-col gap-4">
					{activeRuns.length > 0 && (
						<div className="flex flex-col gap-2 p-4 rounded-2xl bg-success/10 border border-success/20">
							...
							{activeRuns.map((run) => (
								<div
									key={run.run_id}
									className="flex items-center justify-between text-xs text-success-800"
								>
									<span>
										{run.job_name} ({dispatchTargetLabel(run.target)})
									</span>
									<span>
					...
					<div className="grid grid-cols-1 gap-4">
						{dispatchState.data?.jobs.map((job) => {
							const lastRun = mostRecentRunForJob(
								job.id,
								dispatchState.data?.recent_runs ?? [],
							);
							const anchorReset = dispatchAnchorReset(
								job,
								claudeUsage.data,
								codexUsage.data,
							);
							...
							return (
								<Card
									key={job.id}
									className="overflow-hidden border-separator/50 hover:border-primary/30 transition-colors"
								>
