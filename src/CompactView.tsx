...
function staleLabel(
	...
	const countdown =
		retryAfter && new Date(retryAfter).getTime() > Date.now()
			? formatTimeUntil(retryAfter)
			: null;
...
export default function CompactView() {
	...
	return (
		<div className="flex h-screen flex-col bg-background rounded-xl overflow-hidden p-3 gap-2 select-none">
			{/* Header */}
				...
				<CardContent className="p-3 flex flex-col gap-2">
					<div className="flex items-center gap-1.5">
						...
						{claudeUsage.data?.stale && (
							...
							>
								{staleLabel(
									claudeUsage.data.stale_reason,
									claudeUsage.data.retry_after,
								)}
							</Chip>
						)}
					...
					{claudeUsage.error && !claudeUsage.data ? (
						<div className="flex flex-col gap-0.5">
							...
							{claudeErrorRetry && (
								<p className="text-xs text-warning">
									Retry in {formatTimeUntil(claudeErrorRetry)}
								</p>
							)}
						</div>
					) : claudeUsage.loading && !claudeUsage.data ? (
						...
						<>
							<CompactRow
								label="5-Hour"
								remaining={claudeUsage.data.five_hour.remaining}
							/>
							<CompactRow
								label="7-Day"
								remaining={claudeUsage.data.seven_day.remaining}
							/>
						</>
					) : null}
				</CardContent>
				...
				<CardContent className="p-3 flex flex-col gap-2">
					<div className="flex items-center gap-1.5">
						...
						{codexUsage.data?.stale && (
							...
							>
								{staleLabel(
									codexUsage.data.stale_reason,
									codexUsage.data.retry_after,
								)}
							</Chip>
						)}
					...
					{codexUsage.error && !codexUsage.data ? (
						<div className="flex flex-col gap-0.5">
							...
							{codexErrorRetry && (
								<p className="text-xs text-warning">
									Retry in {formatTimeUntil(codexErrorRetry)}
								</p>
							)}
						</div>
					) : codexUsage.loading && !codexUsage.data ? (
						...
						<>
							<CompactRow
								label="5-Hour"
								remaining={codexUsage.data.primary_window.remaining_percent}
							/>
							<CompactRow
								label="7-Day"
								remaining={codexUsage.data.secondary_window.remaining_percent}
							/>
						</>
					) : null}
				</CardContent>
			...
			<div className="flex justify-end pt-1">
				...
				>
					Open TokenMaxx
					<ExternalLinkIcon className="w-2 h-2 ml-1" />
				</Button>
			</div>
		</div>
	);
